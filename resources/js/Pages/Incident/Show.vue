                                ...
                                <div class="inline-block min-w-full py-2 align-middle md:px-6 lg:px-8">
                                    <div class="overflow-hidden shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
                                        <table class="min-w-full mt-4 mb-4 divide-y divide-gray-300">
                                            ...
                                            <tr v-for="(incident,index) in this.incidents">
                                                <td class="whitespace-nowrap py-4 pl-4 pr-3 text-sm sm:pl-6">
                                                    <div class="flex items-center">
                                                        <div class="h-10 w-10 flex-shrink-0">
                                                            <img class="h-10 w-10 rounded-full" :src="incident.avatar" alt="">
                                                        </div>
                                                        <div class="ml-4">
                                                            <div class="font-medium text-gray-900">{{incident.name}}</div>
                                                            <div class="text-gray-500">{{incident.email}}</div>
                                                        </div>
                                                    </div>
                                                </td>
                                                <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                                    <div v-for="(institution,index) in incident.institutions" class="text-gray-900">
                                                        {{institution.description.replace(/_/g," ").toUpperCase()}}
                                                    </div>
                                                </td>
                                                <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                                <span v-if="incident.status==='Active'" class="inline-flex rounded-full bg-green-100 px-2 text-xs font-semibold leading-5 text-green-800">
                                                    Active
                                                </span>
                                                    <span v-if="incident.status!=='Active'" class="inline-flex rounded-full bg-red-100 px-2 text-xs font-semibold leading-5 text-red-800">
                                                    Not-Active
                                                </span>
                                                </td>
                                                <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                                    <div v-for="(role,index) in incident.roles" class="text-gray-900">
